...
static GtkTreeModel * create_palette ()
{
  FILE *fp;
  gchar *filename;
  gchar buf[1024], *name;
  ...
  if (options.color_data.palette) {
    ...
    if (g_file_test ("/etc/X11/rgb.txt", G_FILE_TEST_EXISTS))
        filename = "/etc/X11/rgb.txt";
    else if (g_file_test ("/usr/share/X11/rgb.txt", G_FILE_TEST_EXISTS))
        filename = "/usr/share/X11/rgb.txt";
    else if (g_file_test ("/usr/lib/X11/rgb.txt", G_FILE_TEST_EXISTS))
        filename = "/usr/lib/X11/rgb.txt";
    else if (g_file_test ("/usr/X11R6/lib/X11/rgb.txt", G_FILE_TEST_EXISTS))
        filename = "/usr/X11R6/lib/X11/rgb.txt";
    else if (g_file_test ("/usr/openwin/lib/X11/rgb.txt", G_FILE_TEST_EXISTS))
