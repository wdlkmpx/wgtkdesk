...
static void terminal_tab_set_position(GtkWidget * notebook, gint tab_position)
{
    switch (tab_position)
    {
        case 0:  gtk_notebook_set_tab_pos(GTK_NOTEBOOK(notebook), GTK_POS_TOP);    break;
        case 1:  gtk_notebook_set_tab_pos(GTK_NOTEBOOK(notebook), GTK_POS_BOTTOM); break;
        case 2:  gtk_notebook_set_tab_pos(GTK_NOTEBOOK(notebook), GTK_POS_LEFT);   break;
        default: gtk_notebook_set_tab_pos(GTK_NOTEBOOK(notebook), GTK_POS_RIGHT);  break;
    }
}
...
static void terminal_about_activate_event (GtkMenuItem * item, LXTerminal * terminal)
{
    ...
    GdkPixbuf * logo;
    logo = gdk_pixbuf_new_from_file (PACKAGE_DATADIR "/icons/hicolor/128x128/apps/lxterminal.png", NULL);
...
LXTerminal * lxterminal_initialize(LXTermWindow * lxtermwin, CommandArguments * arguments)
{
    ...
    else
    {
        gtk_window_set_icon_from_file(GTK_WINDOW(terminal->window), 
            PACKAGE_DATADIR "/icons/hicolor/128x128/apps/lxterminal.png", NULL);
    }
