    ...
    if (is_new_color)
    {
        ...
        gtk_tree_selection_select_iter (
            gtk_tree_view_get_selection (
                GTK_TREE_VIEW (lookup_widget (lxcolor, "treeview"))
            ), &iter);
    }
}
...
int main (int argc, char *argv[])
{
    ...
    lxcolor = create_lxcolor ();
    gtk_widget_show_all (lxcolor);
#if GTK_MAJOR_VERSION < 3
    gtk_window_set_policy (GTK_WINDOW (lxcolor), FALSE, FALSE, TRUE);
#endif

    /* setup the tree view widget */
    liststore = gtk_list_store_new (N_COLUMNS, GDK_TYPE_PIXBUF, G_TYPE_STRING,
                                    G_TYPE_STRING, G_TYPE_STRING);
    treeview = lookup_widget (GTK_WIDGET (lxcolor), "treeview");
    ...
    gtk_widget_show (lxcolor);
    gdkwin = gtk_widget_get_window (GTK_WIDGET (lxcolor));
