...
char* print_filesize(file_size_t bytes) {
  ...
  size /= 1024.;
  if (size < 128) return g_strdup_printf("%.2f KiB", size);
  ...
  size /= 1024.;
  if (size < 128) return g_strdup_printf("%.2f EiB", size);
  return g_strdup_printf("%.1f EiB", size);
}
...
int create_dir(const char *dir) {
  ...
  if (!ddir) return 0;
  pos = strrchr(ddir, G_DIR_SEPARATOR);
  if (!pos) return 0;
  *pos = 0;
  ...
  *pos = G_DIR_SEPARATOR;
  ...
  slash = ddir;
  while ((pos = strchr(slash + 1, G_DIR_SEPARATOR)) != NULL) {
    *pos = 0;
    if (!directory_exists(ddir)) {
#ifdef _WIN32
      int err = mkdir(ddir);
#else
      int err = mkdir(ddir, 7 * 64 + 5 * 8 + 5);
#endif
      if (err) {
