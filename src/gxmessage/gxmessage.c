 ...
 * 2012, 2014, 2015 Timothy Richard Musson
 *
...
#define ALLOW_NOESCAPE PACKAGE_DATADIR \
...
gboolean
label_width_okay (const gchar *str)
{
	GtkWidget *dummy;
	PangoLayout *layout;
	int width;
...
window_create (void)
{
	...
	if (gx.font_str != NULL) {
		...
		gtk_widget_override_font (message_widget, font_desc);
	...
	if (gx.color_fg != NULL) {
		...
		if (gdk_rgba_parse (&color, gx.color_fg)) {
			gtk_widget_override_color (message_widget, GTK_STATE_NORMAL, &color);
#else
	...
	if (gx.color_bg != NULL) {
		...
		if (gdk_rgba_parse (&color, gx.color_bg)) {
			gtk_widget_override_background_color (message_widget, GTK_STATE_NORMAL, &color);
#else
	...
	if (gx.do_buttons) {
		...
		while (button != NULL) {
			...
			if (strcmp (button->label, "okay") == 0) {
				/* XXX: gtk_button_new_from_stock deprecated since GTK 3.10 */
	...
	if (gx.message_len > 20000) {
		...
		PangoFontDescription *font_desc;
#if GTK_CHECK_VERSION(3,0,0)
		GtkStyleContext *context = gtk_widget_get_style_context (message_widget);
		gtk_style_context_get (context, GTK_STATE_FLAG_NORMAL, GTK_STYLE_PROPERTY_FONT, &font_desc, NULL);
#else // gtk2
		font_desc = gtk_widget_get_style (message_widget)->font_desc;
#endif
