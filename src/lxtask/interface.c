...
GtkWidget* create_main_window (void)
{
 ...
 //   GtkWidget *button2;
	...
	gtk_widget_add_accelerator(item, "activate", accel_group, 
		GDK_KEY(Escape), (GdkModifierType)0, GTK_ACCEL_VISIBLE);
	gtk_widget_add_accelerator(item, "activate", accel_group, 
		GDK_KEY(W), GDK_CONTROL_MASK, GTK_ACCEL_VISIBLE);
...
//    GtkWidget *trennlinie1;
    ...
    preferences1 = gtk_image_menu_item_new_from_stock("gtk-preferences", NULL);
...
void show_preferences(void)
{
    ...
    GtkWidget *refresh_box = gtk_box_new (GTK_ORIENTATION_HORIZONTAL, 0);
    ...
    dlg = gtk_dialog_new_with_buttons(_("Preferences"), NULL, 0, "gtk-cancel", GTK_RESPONSE_REJECT, "gtk-ok", GTK_RESPONSE_ACCEPT, NULL);
