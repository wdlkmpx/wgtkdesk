...
static GtkWidget *set_scrollable_text (GtkWidget * parent_box)
{
    GtkWidget *text;
#if GTK_CHECK_VERSION(2,0,0)
...
#endif
    w_gtk_widget_set_scrolled_window (text, parent_box);
    /** alternate method with a vertical scrollbar
    GtkWidget *hbox = gtk_hbox_new (FALSE, 0);
    GtkWidget *vscrollbar;
    gtk_box_pack_start (Xdialog.vbox, hbox, TRUE, TRUE, 0);
    // Text with a vertical scrollbar
    vscrollbar = gtk_vscrollbar_new (GTK_TEXT(text)->vadj);
    vscrollbar = gtk_vscrollbar_new (gtk_text_view_get_vadjustment(GTK_TEXT_VIEW(text));
...
static void set_list_size (GtkWidget *widget, gint xsize, gint list_size, gint spacing)
{
    int width, height;
    width = (xsize > 0) ? xsize*xmult : -1;
    if (Xdialog.list_height > 0) {
        height = Xdialog.list_height * (ymult + spacing);
    } else {
        height = MIN(gdk_screen_height() - 40 * ymult, list_size * (ymult + spacing));
    }
...
void create_tailbox(gchar *optarg)
{
    ...
    Xdialog.widget1 = set_scrollable_text (GTK_WIDGET(Xdialog.vbox));
...
void create_logbox(gchar *optarg)
{
    ...
    store = gtk_list_store_new (LOGBOX_NUM_COLS,
                                G_TYPE_STRING,   // date
                                G_TYPE_STRING,   // text
                                GDK_TYPE_COLOR,  // row background color
                                GDK_TYPE_COLOR); // row foreground color
    model = GTK_TREE_MODEL (store);
    listbox = g_object_new (GTK_TYPE_TREE_VIEW,
                            "model", model, NULL);
    ...
    gtk_tree_view_column_set_attributes (col1, renderer,
                                         ...
                                         NULL);
    gtk_tree_view_append_column (GTK_TREE_VIEW(listbox), col1);
    ...
    if (Xdialog.time_stamp)
    {
        ...
        gtk_tree_view_column_set_attributes (col0, renderer,
                                             ...
                                             "foreground-gdk", LOGBOX_COL_FGCOLOR,
                                             NULL);
        gtk_tree_view_insert_column (GTK_TREE_VIEW(listbox), col0, 0);

        xsize = 80;
    }
#else // -- GTK1 --
    listbox = gtk_clist_new (Xdialog.time_stamp ? 2 : 1);
    gtk_clist_set_shadow_type (GTK_CLIST(listbox), GTK_SHADOW_IN);
    if (Xdialog.time_stamp) {
        xsize = (Xdialog.date_stamp ? 69 : 58);
    }
#endif

    Xdialog.widget1 = listbox;
    scrollwin = w_gtk_widget_set_scrolled_window (listbox, GTK_WIDGET(Xdialog.vbox));
    set_list_size (scrollwin, xsize, 12, 2);

    w_gtk_listbox_set_selection_mode (listbox, GTK_SELECTION_NONE);
    if (Xdialog.time_stamp) {
        w_gtk_listbox_set_column_title (listbox, 0, Xdialog.date_stamp ? DATE_STAMP : TIME_STAMP);
        w_gtk_listbox_set_column_title (listbox, 1, LOG_MESSAGE);
        w_gtk_listbox_set_headers_visible (listbox, TRUE);
        w_gtk_listbox_set_headers_clickable (listbox, FALSE);
    } else {
        w_gtk_listbox_set_headers_visible (Xdialog.widget1, FALSE);
    }
...
void create_combobox(gchar *optarg, gchar *options[], gint list_size)
{
    ...
    combo           = w_gtk_combobox_new_with_entry ();
    Xdialog.widget1 = w_gtk_combobox_get_entry (combo);
    ...
    for (i = 0; i < list_size; i++) {
        w_gtk_combobox_append_text (combo, options[i]);
    }
    if (strlen(Xdialog.default_item) != 0) {
        w_gtk_combobox_prepend_text (combo, Xdialog.default_item);
    }
...
{ // radiolist / checklist
    ...
    scrollwin = w_gtk_widget_set_scrolled_window (vbox, GTK_WIDGET(Xdialog.vbox));
    set_list_size (scrollwin, -1, list_size, ymult + 5);
...
void create_menubox (gchar *optarg, gchar *options[], gint list_size)
{
...
#else // -- GTK1 --
    listbox = gtk_clist_new (2);
#endif
    Xdialog.widget2 = listbox;
    scrollwin = w_gtk_widget_set_scrolled_window (listbox, GTK_WIDGET(Xdialog.vbox));
    set_list_size (scrollwin, -1, list_size, 4);

    w_gtk_listbox_set_headers_visible (listbox, FALSE);
    w_gtk_listbox_set_selection_mode (listbox, GTK_SELECTION_BROWSE);
    ...
    gtk_tree_view_set_model (GTK_TREE_VIEW(listbox), tree_model);
    ...
    g_signal_connect (G_OBJECT(listbox),  "row_activated",
    ...
    w_gtk_listbox_select_row (listbox, rownum);
    ...
    for (i = 0; i < list_size; i++)
    {
        ...
        rownum = gtk_clist_append (GTK_CLIST(listbox), null_row);

        if (strlen(Xdialog.array[i].tag) == 0) {
            gtk_clist_set_text (GTK_CLIST(listbox), rownum, 0, "~");
            gtk_clist_set_selectable (GTK_CLIST(listbox), rownum, FALSE);
            gtk_clist_set_foreground (GTK_CLIST(listbox), rownum, (GdkColor *) &GREY1);
            gtk_clist_set_background (GTK_CLIST(listbox), rownum, (GdkColor *) &GREY2);
        } else {
            gtk_clist_set_text (GTK_CLIST(listbox), rownum, 0, Xdialog.array[i].tag);
            gtk_clist_set_row_data (GTK_CLIST(listbox), rownum, (gpointer) &Xdialog.array[i]);
            if (selrow == -1) {
                selrow = rownum; // first selectable row
            }
        }
        gtk_clist_set_text (GTK_CLIST(listbox), rownum, 1, Xdialog.array[i].name);
    ...
    if (selrow >= 0) {
        ...
        w_gtk_listbox_select_row (listbox, selrow);
    }
    ...
    if (!Xdialog.tags) {
        w_gtk_listbox_set_column_visibility (listbox, 0, FALSE);
    }

    g_signal_connect (G_OBJECT(button_ok), "clicked", // GtkTreeView or GtkCList
                      G_CALLBACK(on_menubox_ok_click),   listbox); 
...
void create_treeview (gchar *optarg, gchar *options[], gint list_size)
{
    ...
    scrollwin = w_gtk_widget_set_scrolled_window (Xdialog.widget1, GTK_WIDGET(Xdialog.vbox));
    set_list_size (scrollwin,  -1, list_size, 4);
