                          ...
                          gboolean with_list_height)
{
    ...
    if (list_size * parameters + *optind != next_opt) {
        print_help_info(argv[0], "incorrect number of parameters in list");
    }
    if (list_size < 1) {
        print_help_info(argv[0], "missing list parameters");
    }
...
int main(int argc, char *argv[])
{
    ...
    if (!gtk_init_check(&argc, &argv)) {
        fprintf(stderr, "Error initializing the GUI...");
        return 255;
    }
        ...
        /* Box options */
            ...
            case B_COLORSEL:    /* colour selector */
                ...
                if (list_size != 0) {
                    ...
                    rgb[0] = atoi(argv[optind++]);
                    rgb[1] = atoi(argv[optind++]);
                    rgb[2] = atoi(argv[optind++]);
                }  else {
                    rgb[0] = rgb[1] = rgb[2] = 0;
                }
                create_colorsel(optarg, rgb);
                win = TRUE;
                break;
