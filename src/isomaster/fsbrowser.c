...
void buildFsBrowser(GtkWidget* boxToPackInto)
{
    ...
    * if i do a connect_after the handler never gets called. So no right-click menu. */
    ...
    iconSet = gtk_icon_factory_lookup_default("gtk-directory");
    if(iconSet != NULL)
    {
        ...
        GBLdirPixbuf = gtk_widget_render_icon(GBLfsTreeView, "gtk-directory", iconSize, NULL);
    }
    ...
    if(iconSet != NULL)
    {
        ...
        g_free(iconSizes);
        //!! figure out proper size and resisze if necessary, see gtk-demo->stock->create_model()
        GBLfilePixbuf = gtk_widget_render_icon(GBLfsTreeView, "gtk-file", iconSize, NULL);
    }
                      ...
                      GtkTreeViewColumn* col, gpointer data)
{
    ...
    /* if it's an image, open it */
    {
        ...
        if ((g_ascii_strcasecmp (ext,".iso") == 0)
            || (g_ascii_strcasecmp (ext,".nrg") == 0)
            || (g_ascii_strcasecmp (ext,".mdf") == 0) )
        {
