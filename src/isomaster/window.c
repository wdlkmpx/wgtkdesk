...
#define ICONPATH PACKAGE_DATADIR "/isomaster/icons"
...
void buildMainToolbar(GtkWidget* boxToPackInto)
{
    ...
    w_gtk_toolbar_button_new (toolbar,
                              _("Go back"),
                              _("Go back up one directory on the filesystem"),
                              NULL, GBLgoBackIcon,
                              G_CALLBACK(fsGoUpDirTreeCbk), NULL);

    w_gtk_toolbar_button_new (toolbar,
                              _("New Directory"),
                              _("Create new directory on the filesystem"),
                              NULL, GBLnewDirIcon,
                              G_CALLBACK(createDirCbk), (gpointer)1);
...
void buildMenu(GtkWidget* boxToPackInto)
{
    ...
    menuItem = gtk_image_menu_item_new_from_stock("gtk-new", NULL);
    //~ icon = gtk_image_new_from_stock("gtk-new", GTK_ICON_SIZE_NEW);
    ...
    menuItem = gtk_image_menu_item_new_from_stock("gtk-save", NULL);
    gtk_menu_shell_append(GTK_MENU_SHELL(menu), menuItem);
    gtk_widget_show(menuItem);
    g_signal_connect(G_OBJECT(menuItem), "activate",
    ...
    rootSubmenu2 = gtk_image_menu_item_new_from_stock("gtk-add", NULL);
    ...
    icon = gtk_image_new_from_stock("gtk-help", GTK_ICON_SIZE_MENU);
...
void buildMiddleToolbar(GtkWidget* boxToPackInto)
{
    ...
    w_gtk_toolbar_button_new (toolbar,
                              _("Go back"),
                              _("Go back up one directory on the ISO"),
                              NULL, GBLgoBackIcon2,
                              G_CALLBACK(isoGoUpDirTreeCbk), NULL);

    w_gtk_toolbar_button_new (toolbar,
                              _("New Directory"),
                              _("Create new directory on the ISO"),
                              NULL, GBLnewDirIcon2,
                              G_CALLBACK(createDirCbk), NULL);
    ...
    w_gtk_toolbar_button_new (toolbar,
                              _("Extract"),
                              _("Extract from the ISO"),
                              NULL, GBLextractIcon,
                              G_CALLBACK(extractFromIsoCbk), NULL);

    w_gtk_toolbar_button_new (toolbar,
                              _("Remove"),
                              _("Delete from the ISO"),
                              NULL, GBLdeleteIcon2,
                              G_CALLBACK(deleteFromIsoCbk), NULL);

    sizeTitleLabel = gtk_label_new(_("      Estimated ISO Size: "));
    gtk_box_pack_start (GTK_BOX(toolbar), sizeTitleLabel, FALSE, FALSE, 0);

    GBLisoSizeLbl = gtk_label_new("");
    gtk_box_pack_start (GTK_BOX(toolbar), GBLisoSizeLbl, FALSE, FALSE, 0);
...
void loadIcon(GtkWidget** destIcon, const char* srcFile, int size)
{
    ...
    {
        *destIcon = gtk_image_new_from_stock("gtk-missing-image", GTK_ICON_SIZE_LARGE_TOOLBAR);
    }
    else
