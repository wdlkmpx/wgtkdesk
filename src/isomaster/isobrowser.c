...
void addToIsoCbk(GtkButton *button, gpointer data)
{
    ...
    main_vbox = gtk_dialog_get_content_area (GTK_DIALOG(progressWindow));
    ...
    gtk_box_pack_start(GTK_BOX(main_vbox), GBLactivityProgressBar, TRUE, TRUE, 0);
    gtk_widget_show(GBLactivityProgressBar);
    
    /* button to cancel adding */
    gtk_dialog_add_button(GTK_DIALOG(progressWindow), "gtk-cancel", GTK_RESPONSE_CANCEL);
...
void buildIsoLocator(GtkWidget* boxToPackInto)
{
    ...
    //gtk_editable_set_editable (GTK_EDITABLE(GBLisoCurrentDirField), FALSE);
...
bool confirmCloseIso(void)
{
    ...
    gtk_dialog_add_buttons(GTK_DIALOG(warningDialog),
                           "gtk-go-back", GTK_RESPONSE_CANCEL,
...
{
    GtkTreeSelection* selection;
    GtkWidget* progressWindow = NULL, *main_vbox;
    ...
    if(gtk_tree_selection_count_selected_rows(selection) > 0)
    {
        ...
        gtk_box_pack_start(GTK_BOX(main_vbox), descriptionLabel, TRUE, TRUE, 0);
        ...
        gtk_dialog_add_button(GTK_DIALOG(progressWindow), "gtk-cancel", GTK_RESPONSE_NONE);
...
{
    int rc;
    GtkWidget* warningDialog, *main_vbox;
                          ...
                          GtkTreeIter* iterator, gpointer data)
{
    ...
    gtk_entry_set_max_length (GTK_ENTRY(nameField), NCHARS_FILE_ID_MAX_STORE);
    ...
    main_vbox = gtk_dialog_get_content_area (GTK_DIALOG(dialog));
    gtk_box_pack_start(GTK_BOX(main_vbox), nameField, TRUE, TRUE, 0);
...
void saveIso(char* filename)
{
    ...
    main_vbox = gtk_dialog_get_content_area (GTK_DIALOG(GBLwritingProgressWindow));
    gtk_box_pack_start(GTK_BOX(main_vbox), descriptionLabel, TRUE, TRUE, 0);
    gtk_widget_show(descriptionLabel);
    
    /* the progress bar */
    GBLWritingProgressBar = gtk_progress_bar_new();
    gtk_box_pack_start(GTK_BOX(main_vbox), GBLWritingProgressBar, TRUE, TRUE, 0);
    gtk_widget_show(GBLWritingProgressBar);
    
    /* button to close the dialog (disabled until writing finished) */
    okButton = gtk_dialog_add_button(GTK_DIALOG(GBLwritingProgressWindow), "gtk-ok", GTK_RESPONSE_OK);
    gtk_widget_set_sensitive(okButton, FALSE);
    
    /* button to cancel writing */
    cancelButton = gtk_dialog_add_button(GTK_DIALOG(GBLwritingProgressWindow), "gtk-cancel", GTK_RESPONSE_CANCEL);
