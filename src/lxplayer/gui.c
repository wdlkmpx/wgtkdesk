...
// menuitems that are used outside gui.c
...
static GtkWidget *config_remember_position;
...
static const char *audio_drivers_strv[] =
{
    "",
    "alsa",
    "jack",
#ifndef __linux__
    "oss", // problem with alsa OSS emulation in older Linux distros
#endif
    "pulse",
#ifdef __OpenBSD__
    "sndio",
...
void remember_position_func ()
{
    double pos, len;
    const char *uri;
    if (remember_position)
    {
        pos = gmtk_media_player_get_attribute_double (GMTK_MEDIA_PLAYER(media), ATTRIBUTE_POSITION);
        len = gmtk_media_player_get_attribute_double (GMTK_MEDIA_PLAYER(media), ATTRIBUTE_LENGTH);
        uri = gmtk_media_player_get_uri (GMTK_MEDIA_PLAYER(media));
        //gm_log (TRUE, G_LOG_LEVEL_INFO, "*** length:%lf   --   pos:%lf", len, pos);
        if (pos <= 10.0) {
            resume_cache_remove (uri); // need at least 10 seconds
        } else if (pos >= (len-10.0)) {
            resume_cache_remove (uri); // it's finishing, no need to add
        } else {
            resume_cache_add (uri, pos);
        }
...
{
    switch (media_state) {
    case MEDIA_STATE_PAUSE:   return PAUSED;
    case MEDIA_STATE_PLAY:    return PLAYING;
    case MEDIA_STATE_QUIT:
    case MEDIA_STATE_UNKNOWN: return QUIT;
    case MEDIA_STATE_STOP:    return STOPPED;
    default:                  return QUIT;
    }
}
...
gboolean add_to_playlist_and_play(gpointer data)
{
    ...
    if (gtk_tree_model_iter_n_children(GTK_TREE_MODEL(playliststore), NULL) == 1
        ...
        if (gtk_tree_model_get_iter_first(GTK_TREE_MODEL(playliststore), &iter)) {
            ...
            present_main_window(); //bring_to_front
        }
    }
    if (s != NULL)
        g_free(s);
...
static void view_option_show_callback(GtkWidget * widget, gpointer data)
{
    skip_fixed_allocation_on_show = TRUE;
}

static void view_option_hide_callback(GtkWidget * widget, gpointer data)
{
    skip_fixed_allocation_on_hide = TRUE;
    g_idle_add(set_adjust_layout, NULL);
}

static void view_option_size_allocate_callback(GtkWidget * widget, GtkAllocation * allocation, gpointer data)
{
...
static void player_size_allocate_callback(GtkWidget * widget, GtkAllocation * allocation)
{
    ...
    if (data != NULL && idle != NULL && gtk_widget_get_visible(window)) {
        ...
        if (total > 1) {
            if (name == NULL || strlen(name) < 1) {
                buf = g_strdup_printf(_("(%i/%i) - LXplayer"), current + 1, total);
            } else {
                buf = g_strdup_printf(_("%s - (%i/%i) - LXplayer"), name, current + 1, total);
            }
        } else {
            if (name == NULL || strlen(name) < 1) {
                buf = g_strdup_printf(_("LXplayer"));
            } else {
                buf = g_strdup_printf(_("%s - LXplayer"), name);
            }
        }
...
{
    gdouble vol = gtk_scale_button_get_value(GTK_SCALE_BUTTON(vol_slider));
...
gboolean set_gui_state(void *data)
{
    ...
    gm_log(TRUE, G_LOG_LEVEL_INFO, "setting gui state to %s", PLAYSTATE_to_string(guistate));
    if (lastguistate != guistate)
    {
        if (guistate == PLAYING) {
            w_gtk_image_set_from_icon_name (GTK_IMAGE(image_play), icon_pause, button_size);
        ...
        if (guistate == PAUSED) {
            w_gtk_image_set_from_icon_name (GTK_IMAGE(image_play), icon_play, button_size);
...
{
    gtk_window_present(GTK_WINDOW(window)); //bring_to_front
...
/*static*/ void set_software_volume (gdouble vol)
{
    // remember_softvol
    volume_softvol = vol;
    gm_store = gm_pref_store_new("lxplayer");
    gm_pref_store_set_float(gm_store, VOLUME_SOFTVOL, vol);
...
gboolean set_volume(void *data)
{
    ...
    if (data == NULL) {
        gm_log(verbose, G_LOG_LEVEL_DEBUG, "in set_volume without data - doing nothing");
        return FALSE;
    }
    if (GTK_IS_WIDGET(vol_slider) && audio_device.volume >= 0.0 && audio_device.volume <= 1.0) {
        ...
        set_software_volume (audio_device.volume);
...
{
    gtk_main_quit(); // exit gtk main loop, see main() - main.c
}
...
void menuitem_about_callback(GtkMenuItem * menuitem, void *data)
{
    ...
    const gchar * authors[] =
    {
        "Kevin DeKorte - original author", "James Carthew", "Diogo Franco", "wdlkmpx",
        NULL
    };
...
void config_apply(GtkWidget * widget, void *data)
{
    ...
    if (audio_device.mplayer_ao) {
        g_free(audio_device.mplayer_ao);
    }

    audio_device.mplayer_ao = g_strdup (gtk_combo_box_text_get_active_text (GTK_COMBO_BOX_TEXT (config_ao)));
    ...
    remember_position = gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(config_remember_position));
    ...
    if (GTK_IS_STATUS_ICON(status_icon)) {
        ...
        if (show_status_icon) {
            ...
            if (gtk_icon_theme_has_icon(icon_theme, "lxplayer")) {
                status_icon = gtk_status_icon_new_from_icon_name("lxplayer");
            } else {
    ...
    gm_pref_store_set_string(gm_store, AUDIO_DEVICE_NAME, audio_device.mplayer_ao);
    ...
    gm_pref_store_set_boolean(gm_store, REMEMBER_POSITION, remember_position);
...
static void create_details_table (GtkWidget *vbox)
{
    ...
    grid.table = details_table = w_gtk_grid_new (vbox, 0);
...
void menuitem_video_adjustments_dlg (GtkMenuItem * menuitem, void *data)
{
    ...
    GtkWidget * img = gtk_image_new_from_icon_name ("gtk-close", button_size);
...
void menuitem_view_aspect_callback (GtkMenuItem * menuitem, void *data)
{
    ...
    // - "index" is set by w_gtk_menu_item_new() - GtkMenu
    int i = 0;
    int active;
...
gchar *osdlevel_format_callback(GtkScale * scale, gdouble value)
{
    ...
    switch ((gint) value)
    {
    case 0:  text = g_strdup(_("No Display")); break;
    case 1:  text = g_strdup(_("Minimal")); break;
    case 2:  text = g_strdup(_("Timer")); break;
    case 3:  text = g_strdup(_("Timer/Total")); break;
    default: text = g_strdup("How did we get here?");
    }
    return text;
}
...
gchar *pplevel_format_callback(GtkScale * scale, gdouble value)
{
    ...
    switch ((gint) value)
    {
    case 0:  text = g_strdup (_("No Postprocessing")); break;
    case 1:
    case 2:  text = g_strdup (_("Minimal Postprocessing")); break;
    case 3:
    case 4:  text = g_strdup (_("More Postprocessing")); break;
    case 5:
    case 6:  text = g_strdup (_("Maximum Postprocessing")); break;
    default: text = g_strdup ("error (value)");
    }
    return text;
}
...
gchar *subtitle_fuzziness_format_callback(GtkScale * scale, gdouble value)
{
    ...
    switch ((gint) value)
    {
    case 0:  text = g_strdup (_("Exact Match")); break;
    case 1:  text = g_strdup (_("Load all subtitles containing movie name")); break;
    case 2:  text = g_strdup (_("Load all subtitles in the same folder")); break;
    default: text = g_strdup ("error (value)");
    }
    return text;
}
...
void menuitem_config_dialog_cb(GtkMenuItem * menuitem, void *data)
{
    ...
    for (i = 0; i < 4 ; i++)
    {
        page_table[i] = w_gtk_notebook_add_tab_grid (notebook, gettext(tab_labels[i]));
    }

    gtk_container_add(GTK_CONTAINER(conf_vbox), notebook);
    gtk_container_add(GTK_CONTAINER(config_window), conf_vbox);

    gtk_window_set_title(GTK_WINDOW(config_window), _("LXplayer Configuration"));
    ...
    w_gtk_combobox_populate_from_strv (config_vo, vo_strv, -1, FALSE);
    w_gtk_combobox_select_or_prepend (config_vo, vo);
    ...
    config_ao = gtk_combo_box_text_new ();
    w_gtk_combobox_populate_from_strv (config_ao, audio_drivers_strv, -1, FALSE);
    if (audio_device.mplayer_ao && *audio_device.mplayer_ao) {
        w_gtk_combobox_select_or_prepend (config_ao, audio_device.mplayer_ao);
    }
    ...
    w_gtk_combobox_populate_from_strv (config_alang, langlist_strv, -1, FALSE);
    w_gtk_combobox_select_or_prepend (config_alang, alang);
    ...
    w_gtk_combobox_populate_from_strv (config_slang, langlist_strv, -1, FALSE);
    w_gtk_combobox_select_or_prepend (config_slang, slang);
    ...
    w_gtk_combobox_populate_from_strv (config_metadata_codepage, codepagelist_strv, 0, FALSE);
    w_gtk_combobox_select_or_prepend (config_metadata_codepage, metadata_codepage);

    config_subtitle_codepage = gtk_combo_box_text_new_with_entry();
    w_gtk_combobox_populate_from_strv (config_subtitle_codepage, codepagelist_strv, 0, FALSE);
    w_gtk_combobox_select_or_prepend (config_subtitle_codepage, subtitle_codepage);

    config_audio_channels = gtk_combo_box_text_new ();
    ...
    w_gtk_combobox_populate_from_strv (config_audio_channels, channels_strv, audio_channels, FALSE);
    ...
    grid.c2.w = gtk_check_button_new_with_mnemonic(_("De_interlace Video"));
    config_deinterlace = grid.c2.w;
    ...
    grid.c2.w = gtk_check_button_new_with_mnemonic(_("_Drop frames"));
    config_framedrop = grid.c2.w;
    ...
    grid.c2.w = gtk_spin_button_new_with_range (0.50, 10, 0.05);
    ...
    gtk_entry_set_alignment(GTK_ENTRY(config_subtitle_scale), 1);
    gtk_editable_set_editable(GTK_EDITABLE(config_subtitle_scale), FALSE);
    //gtk_entry_set_width_chars(GTK_ENTRY(config_subtitle_scale), 6);
    w_gtk_grid_append_row (&grid);
    ...
    grid.c1.w = gtk_check_button_new_with_label(_("Remember time position (of the last 50 opened files)"));
    config_remember_position = grid.c1.w;
    gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(config_remember_position), remember_position);
    w_gtk_grid_append_row (&grid);
    ...
    grid.c1.w = gtk_check_button_new_with_label(_("mplayer cache (size in KB)"));
    ...
    gtk_widget_set_tooltip_text (config_cachesize, _("Amount of data to cache when playing media, use higher values for slow devices and sites."));
    ...
    grid.c1.w = gtk_label_new(_("Place the cursor in the box next to the shortcut you want to modify\n"
    ...
    case ATTRIBUTE_ALBUM:
        ...
        if (gmtk_media_player_get_attribute_string(GMTK_MEDIA_PLAYER(media), ATTRIBUTE_TITLE)) {
            ...
            if (metadata->title != NULL && strlen(metadata->title) > 0) {
                title = g_strdup_printf(_("%s - LXplayer"), metadata->title);
...
void make_button(gchar * src, gchar * hrefid)
{
    ...
    if (error != NULL) {
        g_error_free(error);
        error = NULL;
        basepath = g_strdup_printf("%s/lxplayer/plugin", g_get_user_cache_dir());
        dirname = gm_tempname(basepath, "lxplayerXXXXXX");
...
GtkWidget *create_window()
{
    ...
    gtk_window_set_title(GTK_WINDOW(window), _("LXplayer"));
    gtk_window_set_resizable(GTK_WINDOW(window), TRUE);
    ...
    image_play = w_gtk_image_new_from_icon_name (icon_play, button_size);
    image_fs   = w_gtk_image_new_from_icon_name (icon_fs,   button_size);

    if (gtk_icon_theme_has_icon(icon_theme, "lxplayer")) {
        gtk_window_set_default_icon_name ("lxplayer");
        pb_icon = gtk_icon_theme_load_icon (icon_theme, "lxplayer", 64, 0, NULL);
    } else {
    ...
    if (show_status_icon) {
        if (gtk_icon_theme_has_icon(icon_theme, "lxplayer-panel")) {
            status_icon = gtk_status_icon_new_from_icon_name("lxplayer-panel");
        } else if (gtk_icon_theme_has_icon(icon_theme, "lxplayer")) {
            status_icon = gtk_status_icon_new_from_icon_name("lxplayer");
        } else {
            status_icon = gtk_status_icon_new_from_pixbuf(pb_icon);
        }
    ...
    gtk_scale_button_set_value(GTK_SCALE_BUTTON(vol_slider), audio_device.volume);
    g_object_set(G_OBJECT(vol_slider), "size", button_size, NULL);
