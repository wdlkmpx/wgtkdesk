...
{
    switch (media_state) {
    case MEDIA_STATE_UNKNOWN: return "unknown";
    case MEDIA_STATE_PLAY:    return "play";
    case MEDIA_STATE_PAUSE:   return "pause";
    case MEDIA_STATE_STOP:    return "stop";
    case MEDIA_STATE_QUIT:    return "quit";
...
#define GMPLS_LEN 1024
    ...
    case PLAYER_STATE_DEAD:    g_strlcat(msg, "dead", GMPLS_LEN); break;
    case PLAYER_STATE_RUNNING: g_strlcat(msg, "running", GMPLS_LEN); break;
    default:
        g_strlcat(msg, "???", GMPLS_LEN);
    }
...
static void gmtk_media_player_size_allocate(GtkWidget * widget, GtkAllocation * allocation)
{
    ...
    if (player->video_width == 0 || player->video_height == 0 || !gtk_widget_get_realized(widget)) {
        gtk_alignment_set(GTK_ALIGNMENT(player->alignment), 0.0, 0.0, 1.0, 1.0);
    } else {
        switch (player->aspect_ratio) {
        case ASPECT_4X3:   video_aspect = 4.0 / 3.0; break;
        case ASPECT_16X9:  video_aspect = 16.0 / 9.0; break;
        case ASPECT_16X10: video_aspect = 16.0 / 10.0; break;
        case ASPECT_WINDOW:video_aspect = (gdouble) allocation->width / (gdouble) allocation->height; break;
        case ASPECT_ANAMORPHIC: video_aspect = 2.39; break;
        case ASPECT_DEFAULT:
        default:
...
void gmtk_media_player_set_state(GmtkMediaPlayer * player, const GmtkMediaPlayerMediaState new_media_state)
{
    ...
    if (player->player_state == PLAYER_STATE_DEAD)
    {
        if (new_media_state == MEDIA_STATE_QUIT
            || new_media_state == MEDIA_STATE_STOP
            || new_media_state == MEDIA_STATE_PAUSE)
        {
            player->media_state = MEDIA_STATE_UNKNOWN;
        }
        ...
        {   // launch player
            ...
            if (player->mplayer_thread != NULL) {
                ...
                player->message = g_strdup (gettext("Loading..."));
                if (!player->restart) {
...
void gmtk_media_player_send_command(GmtkMediaPlayer * player, GmtkMediaPlayerCommand command)
{
    ...
    switch (command)
    {
    case COMMAND_SHOW_DVD_MENU:   write_to_mplayer(player, "dvdnav 5\n"); break;
    case COMMAND_TAKE_SCREENSHOT: write_to_mplayer(player, "screenshot 0\n"); break;
    case COMMAND_SWITCH_ANGLE:    write_to_mplayer(player, "switch_angle\n"); break;
    case COMMAND_SWITCH_AUDIO:    write_to_mplayer(player, "switch_audio\n"); break;
    case COMMAND_SUBTITLE_STEP_FORWARD:  write_to_mplayer(player, "sub_step 1\n"); break;
    case COMMAND_SUBTITLE_STEP_BACKWARD: write_to_mplayer(player, "sub_step -1\n"); break;
    case COMMAND_SUBTITLE_SELECT:        gmtk_media_player_cycle_subtitles(player); break;
    ...
    case COMMAND_SWITCH_FRAME_DROP:
        write_to_mplayer(player, "frame_drop\n");
        cmd = g_strdup_printf("osd_show_property_text \"%s ${framedropping}\" 1000 1\n",
                              gettext("Frame Dropping"));
        write_to_mplayer(player, cmd);
        g_free(cmd);
        break;
                                             ...
                                             GmtkMediaPlayerMediaAttributes attribute, gboolean value)
{
            ...
            if (value) {
                cmd =
                    g_strdup_printf("osd_show_property_text \"%s\" 1000 1\n",
                                    gettext("Subtitles Visible"));
            } else {
                cmd =
                    g_strdup_printf("osd_show_property_text \"%s\" 1000 1\n",
                                    gettext("Subtitles Hidden"));
            }
            write_to_mplayer(player, cmd);
            g_free(cmd);
            cmd = NULL;
    ...
    default:
        gm_log(player->debug, G_LOG_LEVEL_INFO, "Unsupported Attribute (set_boolean)");
    }
...
gboolean gmtk_media_player_get_attribute_boolean(GmtkMediaPlayer * player, GmtkMediaPlayerMediaAttributes attribute)
{
    ...
    switch (attribute) {
    case ATTRIBUTE_SUB_VISIBLE:   ret = player->sub_visible; break;
    case ATTRIBUTE_ENABLE_FRAME_DROP: ret = player->frame_drop; break;
    case ATTRIBUTE_SUBS_EXIST:    ret = g_list_length(player->subtitles); break;
    case ATTRIBUTE_PLAYLIST:      ret = player->playlist; break;
    case ATTRIBUTE_SEEKABLE:      ret = player->seekable; break;
    case ATTRIBUTE_HAS_CHAPTERS:  ret = player->has_chapters; break;
    case ATTRIBUTE_TITLE_IS_MENU: ret = player->title_is_menu; break;
    case ATTRIBUTE_DISABLE_UPSCALING: ret = player->disable_upscaling; break;
    case ATTRIBUTE_VIDEO_PRESENT: ret = player->video_present; break;
    case ATTRIBUTE_MUTED:         ret = player->muted; break;
    case ATTRIBUTE_ENABLE_EMBEDDED_FONTS:     ret = player->enable_embedded_fonts; break;
    case ATTRIBUTE_SUBTITLE_OUTLINE: ret = player->subtitle_outline; break;
    case ATTRIBUTE_SUBTITLE_SHADOW:  ret = player->subtitle_shadow; break;
    case ATTRIBUTE_DEINTERLACE:      ret = player->deinterlace; break;
    case ATTRIBUTE_ENABLE_DEBUG:     ret = player->debug; break;
    case ATTRIBUTE_RETRY_ON_FULL_CACHE:     ret = player->retry_on_full_cache; break;
    case ATTRIBUTE_ENABLE_HARDWARE_CODECS:  ret = player->enable_hardware_codecs; break;
    case ATTRIBUTE_ENABLE_CRYSTALHD_CODECS: ret = player->enable_crystalhd_codecs; break;
    case ATTRIBUTE_FORCE_CACHE:             ret = player->force_cache; break;
    default:
        gm_log(player->debug, G_LOG_LEVEL_INFO, "Unsupported Attribute (get_boolean)");
    }
    return ret;
}
                                            ...
                                            GmtkMediaPlayerMediaAttributes attribute, gdouble value)
{
    ...
    switch (attribute)
    {
    case ATTRIBUTE_CACHE_SIZE: player->cache_size = value; break;
    case ATTRIBUTE_START_TIME: player->start_time = value; break;
    case ATTRIBUTE_RUN_TIME:   player->run_time = value; break;
    case ATTRIBUTE_ZOOM:       player->zoom = CLAMP(value, 0.1, 10.0); break;
    case ATTRIBUTE_VOLUME_GAIN: player->volume_gain = CLAMP(value, -200.0, 60.0); break;
    ...
    default:
        gm_log(player->debug, G_LOG_LEVEL_INFO, "Unsupported Attribute (set_double)");
    }
...
gdouble gmtk_media_player_get_attribute_double(GmtkMediaPlayer * player, GmtkMediaPlayerMediaAttributes attribute)
{
    ...
    switch (attribute) {
    case ATTRIBUTE_LENGTH:   ret = player->length; break;
    case ATTRIBUTE_POSITION: ret = player->position; break;
    ...
    case ATTRIBUTE_START_TIME: ret = player->start_time; break;
    case ATTRIBUTE_WIDTH:      ret = (gdouble) player->video_width; break;
    case ATTRIBUTE_HEIGHT:     ret = (gdouble) player->video_height; break;
    case ATTRIBUTE_SUB_COUNT:  ret = (gdouble) g_list_length(player->subtitles); break;
    case ATTRIBUTE_AUDIO_TRACK_COUNT: ret = (gdouble) g_list_length(player->audio_tracks); break;
    case ATTRIBUTE_ZOOM:       ret = player->zoom; break;
    case ATTRIBUTE_SPEED_MULTIPLIER: ret = player->speed_multiplier; break;
    case ATTRIBUTE_SPEED_SET:      ret = player->speed; break;
    case ATTRIBUTE_SUBTITLE_SCALE: ret = player->subtitle_scale; break;
    case ATTRIBUTE_SUBTITLE_DELAY: ret = player->subtitle_delay; break;
    case ATTRIBUTE_AUDIO_DELAY:    ret = player->audio_delay; break;
    case ATTRIBUTE_VIDEO_FPS:      ret = player->video_fps; break;
    case ATTRIBUTE_VOLUME_GAIN:    ret = player->volume_gain; break;
    case ATTRIBUTE_CACHE_PERCENT:  ret = player->cache_percent; break;
    default:
        gm_log(player->debug, G_LOG_LEVEL_INFO, "Unsupported Attribute (get_double)");
    }
    return ret;
}
                                            ...
                                            GmtkMediaPlayerMediaAttributes attribute, const gchar * value)
{
    ...
    default:
        gm_log(player->debug, G_LOG_LEVEL_INFO, "Unsupported Attribute (set_string)");
    }
}
...
const gchar *gmtk_media_player_get_attribute_string(GmtkMediaPlayer * player, GmtkMediaPlayerMediaAttributes attribute)
{
    ...
    default:
        gm_log(player->debug, G_LOG_LEVEL_INFO, "Unsupported Attribute (get_string)");
    }
                                             ...
                                             gint value)
{
    ...
    default:
        gm_log(player->debug, G_LOG_LEVEL_INFO, "Unsupported Attribute (set_integer)");
    }
                                                   ...
                                                   gint delta)
{
    gint value;
    switch (attribute) {
    case ATTRIBUTE_BRIGHTNESS: value = player->brightness + delta; break;
    case ATTRIBUTE_CONTRAST:   value = player->contrast + delta; break;
    case ATTRIBUTE_GAMMA:      value = player->gamma + delta; break;
    case ATTRIBUTE_HUE:        value = player->hue + delta; break;
    case ATTRIBUTE_SATURATION: value = player->saturation + delta; break;
    default:
        return;
    }
...
gint gmtk_media_player_get_attribute_integer(GmtkMediaPlayer * player, GmtkMediaPlayerMediaAttributes attribute)
{
    ...
    switch (attribute) {
    case ATTRIBUTE_BRIGHTNESS: ret = player->brightness; break;
    case ATTRIBUTE_CONTRAST:   ret = player->contrast; break;
    case ATTRIBUTE_GAMMA:      ret = player->gamma; break;
    case ATTRIBUTE_HUE:        ret = player->hue; break;
    case ATTRIBUTE_SATURATION: ret = player->saturation; break;
    case ATTRIBUTE_WIDTH:      ret = player->video_width; break;
    case ATTRIBUTE_HEIGHT:     ret = player->video_height;break;
    case ATTRIBUTE_SUBTITLE_MARGIN:       ret = player->subtitle_margin; break;
    case ATTRIBUTE_SUBTITLE_POSITION:     ret = player->subtitle_position; break;
    case ATTRIBUTE_POST_PROCESSING_LEVEL: ret = player->post_processing_level; break;
    case ATTRIBUTE_SUBTITLE_FUZZINESS:    ret = player->subtitle_fuzziness; break;
    case ATTRIBUTE_CHAPTERS:      ret = player->chapters; break;
    case ATTRIBUTE_VIDEO_BITRATE: ret = player->video_bitrate; break;
    case ATTRIBUTE_AUDIO_BITRATE: ret = player->audio_bitrate; break;
    case ATTRIBUTE_AUDIO_RATE:    ret = player->audio_rate; break;
    case ATTRIBUTE_AUDIO_NCH:     ret = player->audio_nch; break;
    case ATTRIBUTE_OSDLEVEL:      ret = player->osdlevel; break;
    default:
        return 0;
    }
    return ret;
...
static const gchar *playback_error_to_string(const GmtkMediaPlayerPlaybackError playback_error)
{
    ...
    case ERROR_RETRY_WITH_PLAYLIST:return "RETRY_WITH_PLAYLIST";
    case ERROR_RETRY_WITH_HTTP:    return "RETRY_WITH_HTTP";
    case ERROR_RETRY_WITH_MMSHTTP: return "RETRY_WITH_MMSHTTP";
    case ERROR_RETRY_WITHOUT_XVMC: return "RETRY_WITHOUT_XVMC";
    case ERROR_RETRY_ALSA_BUSY:    return "RETRY_ALSA_BUSY";
    case ERROR_RETRY_VDPAU:        return "RETRY_VDPAU";
    case ERROR_RETRY:              return "RETRY";
    case ERROR_RETRY_WITHOUT_DIVX_VDPAU:      return "RETRY_WITHOUT_DIVX_VDPAU";
    case ERROR_RETRY_WITH_HTTP_AND_PLAYLIST:  return "RETRY_WITH_HTTP_AND_PLAYLIST";
    case ERROR_RETRY_WITHOUT_HARDWARE_CODECS: return "RETRY_WITHOUT_HARDWARE_CODECS";
    default:
        return "???";
    }
...
gpointer launch_mplayer(gpointer data)
{
    ...
    do {
        ...
        if (player->mplayer_binary == NULL || !g_file_test(player->mplayer_binary, G_FILE_TEST_EXISTS)) {
            argv[argn++] = g_strdup ("mplayer");
        } else {
            argv[argn++] = g_strdup (player->mplayer_binary);
        }

        // use the profile to set up some default values
        if (player->profile != NULL) {
            argv[argn++] = g_strdup ("-profile");
            argv[argn++] = g_strdup (player->profile);
        }

        if (player->vo != NULL) {
            argv[argn++] = g_strdup ("-vo");
            ...
            if (vodesc_looks_like_vo(player->vo, "vdpau")) {
                ...
                if (player->enable_hardware_codecs) {
                    if (player->enable_divx) {
                        codecs_vdpau = g_strdup ("ffmpeg12vdpau,ffh264vdpau,ffwmv3vdpau,ffvc1vdpau,ffodivxvdpau,");
                    } else {
                        codecs_vdpau = g_strdup ("ffmpeg12vdpau,ffh264vdpau,ffwmv3vdpau,ffvc1vdpau,");
                    }
                }

            } else if (vodesc_looks_like_vo(player->vo, "vaapi")) {
                argv[argn++] = g_strdup (player->vo);
                argv[argn++] = g_strdup ("-va");
                argv[argn++] = g_strdup ("vaapi");

            } else if (vodesc_looks_like_vo(player->vo, "xvmc")) {

                if (player->disable_xvmc) {
                    argv[argn++] = g_strdup ("xv,");
                } else {
        ...
        if (player->deinterlace) {
            argv[argn++] = g_strdup ("-vf-pre");
            argv[argn++] = g_strdup ("yadif,softskip,scale");
        }

        if (player->post_processing_level > 0) {
            argv[argn++] = g_strdup ("-vf-add");
            argv[argn++] = g_strdup ("pp=ac/tn:a");
            argv[argn++] = g_strdup ("-autoq");
            argv[argn++] = g_strdup_printf("%d", player->post_processing_level);
        }

        argv[argn++] = g_strdup ("-vf-add");
        argv[argn++] = g_strdup ("screenshot");

        if (player->enable_crystalhd_codecs) {
            codecs_crystalhd = g_strdup ("ffmpeg2crystalhd,ffdivxcrystalhd,ffwmv3crystalhd,ffvc1crystalhd,ffh264crystalhd,ffodivxcrystalhd,");
        }
        ...
        if (codecs != NULL) {
            argv[argn++] = g_strdup ("-vc");
            argv[argn++] = g_strdup (codecs);
            g_free(codecs);
            codecs = NULL;
        }
        ...
        if (!player->ao || !*player->ao) {
            argv[argn++] = g_strdup ("-ao");
            // extra ",": try other drivers after trying alsa...
            argv[argn++] = g_strdup ("pulse,jack,alsa,");
        }
#endif

        if (player->ao && *player->ao) {
            argv[argn++] = g_strdup ("-ao");
            argv[argn++] = g_strdup (player->ao);

            if (player->alsa_mixer && *player->alsa_mixer) {
                argv[argn++] = g_strdup ("-mixer-channel");
                argv[argn++] = g_strdup (player->alsa_mixer);
            }
        }

        if (player->audio_channels > 0)
        { // see config_audio_channels combo and channels_strv[] in gui.c
            argv[argn++] = g_strdup ("-channels");
            switch (player->audio_channels) {
            case 2:  argv[argn++] = g_strdup ("4"); break;
            case 3:  argv[argn++] = g_strdup ("6"); break;
            case 4:  argv[argn++] = g_strdup ("8"); break;
            default: argv[argn++] = g_strdup ("2"); break;
            }
        }

        argv[argn++] = g_strdup ("-af-add");
        argv[argn++] = g_strdup_printf("export=%s:512", player->af_export_filename);

        argv[argn++] = g_strdup ("-quiet");
        argv[argn++] = g_strdup ("-slave");
        argv[argn++] = g_strdup ("-noidle");
        argv[argn++] = g_strdup ("-noconsolecontrols");
        argv[argn++] = g_strdup ("-nostop-xscreensaver");
        argv[argn++] = g_strdup ("-identify");

        // softvol is always enabled
        argv[argn++] = g_strdup ("-softvol");

        if ((gint) (player->volume * 100) != 0) {
            argv[argn++] = g_strdup ("-volume");
        ...
        if (player->frame_drop) {
            argv[argn++] = g_strdup ("-framedrop");
        }
        argv[argn++] = g_strdup ("-msglevel");
        argv[argn++] = g_strdup ("all=5");

        argv[argn++] = g_strdup ("-osdlevel");
        argv[argn++] = g_strdup_printf("%i", player->osdlevel);

        argv[argn++] = g_strdup ("-delay");
        tmp = g_new0(char, G_ASCII_DTOSTR_BUF_SIZE);
        tmp = g_ascii_dtostr(tmp, G_ASCII_DTOSTR_BUF_SIZE, player->audio_delay);
        argv[argn++] = g_strdup(tmp);
        g_free(tmp);

        argv[argn++] = g_strdup ("-subdelay");
        tmp = g_new0(char, G_ASCII_DTOSTR_BUF_SIZE);
        tmp = g_ascii_dtostr(tmp, G_ASCII_DTOSTR_BUF_SIZE, player->subtitle_delay);
        argv[argn++] = g_strdup(tmp);
        g_free(tmp);

        argv[argn++] = g_strdup ("-subpos");
        argv[argn++] = g_strdup_printf("%i", player->subtitle_position);

        argv[argn++] = g_strdup ("-sub-fuzziness");
        ...
        argv[argn++] = g_strdup ("-wid");
        argv[argn++] = g_strdup_printf("0x%x", player->socket_id);

        argv[argn++] = g_strdup ("-brightness");
        argv[argn++] = g_strdup_printf("%i", player->brightness);
        argv[argn++] = g_strdup ("-contrast");
        argv[argn++] = g_strdup_printf("%i", player->contrast);
        //argv[argn++] = g_strdup ("-gamma");
        //argv[argn++] = g_strdup_printf("%i", player->gamma);
        argv[argn++] = g_strdup ("-hue");
        argv[argn++] = g_strdup_printf("%i", player->hue);
        argv[argn++] = g_strdup ("-saturation");
        argv[argn++] = g_strdup_printf("%i", player->saturation);

        if (player->alang && *player->alang) {
            argv[argn++] = g_strdup ("-alang");
            argv[argn++] = g_strdup (player->alang);
        }

        if (player->slang && *player->slang) {
            argv[argn++] = g_strdup ("-slang");
            argv[argn++] = g_strdup (player->slang);
        }

        /* disable msg stuff to make sure extra console characters don't mess around */
        argv[argn++] = g_strdup ("-nomsgcolor");
        argv[argn++] = g_strdup ("-nomsgmodule");

        // mplayer says that nokeepaspect* isn't supported by all vo's but it seems to work
        argv[argn++] = g_strdup ("-nokeepaspect");

        if (player->audio_track_file && *player->audio_track_file) {
            argv[argn++] = g_strdup ("-audiofile");
            argv[argn++] = g_strdup (player->audio_track_file);
        }

        if (player->subtitle_file && *player->subtitle_file) {
            argv[argn++] = g_strdup ("-sub");
            argv[argn++] = g_strdup (player->subtitle_file);
        }

        // subtitles, always enable ASS subs, recompile mplayer if needed...
        argv[argn++] = g_strdup ("-ass");

        if (player->subtitle_margin > 0) {
            argv[argn++] = g_strdup ("-ass-bottom-margin");
            argv[argn++] = g_strdup_printf("%i", player->subtitle_margin);
            argv[argn++] = g_strdup ("-ass-use-margins");
        }

        if (player->enable_embedded_fonts) {
            argv[argn++] = g_strdup ("-embeddedfonts");
        } else {
            argv[argn++] = g_strdup ("-noembeddedfonts");

            if (player->subtitle_font && *player->subtitle_font)
            {
                ...
                if (size) *size = '\0';
                argv[argn++] = g_strdup ("-ass-force-style");
        ...
        if (player->subtitle_color && *player->subtitle_color) {
            argv[argn++] = g_strdup ("-ass-color");
            argv[argn++] = g_strdup (player->subtitle_color);
        }

        if (player->subtitle_scale >= 0.5) {
            //argv[argn++] = g_strdup ("-ass-font-scale");
            //argv[argn++] = g_strdup_printf("%d", (int) (player->subtitle_scale));
            // this applies to SRT and anything that is not SSA/ASS
            argv[argn++] = g_strdup ("-subfont-text-scale");
            // add 2 to scale (somehow fixes discrepancy between ASS/SRT subs
            argv[argn++] = g_strdup_printf("%d", (int) (player->subtitle_scale) + 2);
        }
        ...
        if (player->subtitle_font && *player->subtitle_font) {
            ...
            size = g_strrstr(fontname, " ");
            if (size) size[0] = '\0';
            argv[argn++] = g_strdup ("-subfont");
            argv[argn++] = g_strdup (fontname);
            g_free(fontname);
        }

        if (player->subtitle_codepage && *player->subtitle_codepage) {
            argv[argn++] = g_strdup ("-subcp");
            argv[argn++] = g_strdup (player->subtitle_codepage);
        }
        ...
        case TYPE_FILE:
            if (filename != NULL) {
                if (player->force_cache && player->cache_size >= 32) {
                    argv[argn++] = g_strdup ("-cache");
                    argv[argn++] = g_strdup_printf("%i", (gint) player->cache_size);
                }
                if (player->playlist) {
                    argv[argn++] = g_strdup ("-playlist");
                }
                argv[argn++] = g_strdup (filename);
                break;
            }
            break;
        case TYPE_CD:
            argv[argn++] = g_strdup ("-cache");
            argv[argn++] = g_strdup_printf("%i", (gint) player->cache_size);
            argv[argn++] = g_strdup (player->uri);
            if (player->media_device != NULL) {
                argv[argn++] = g_strdup ("-dvd-device");
                argv[argn++] = g_strdup (player->media_device);
            }
            break;

        case TYPE_DVD:
            argv[argn++] = g_strdup ("-mouse-movements");
            argv[argn++] = g_strdup ("-nocache");
            argv[argn++] = g_strdup ("dvdnav://");
            if (player->media_device != NULL) {
                argv[argn++] = g_strdup ("-dvd-device");
                argv[argn++] = g_strdup (player->media_device);
            }
        ...
        case TYPE_NETWORK:
            if (g_strrstr(player->uri, "apple.com")) {
                argv[argn++] = g_strdup ("-user-agent");
                argv[argn++] = g_strdup ("QuickTime/7.6.9");
            }
        ...
        case TYPE_TV:
            if (player->tv_device != NULL) {
                argv[argn++] = g_strdup ("-tv:device");
                argv[argn++] = g_strdup (player->tv_device);
            }
            if (player->tv_driver != NULL) {
                argv[argn++] = g_strdup ("-tv:driver");
                argv[argn++] = g_strdup (player->tv_driver);
            }
            if (player->tv_input != NULL) {
                argv[argn++] = g_strdup ("-tv:input");
                argv[argn++] = g_strdup (player->tv_input);
            }
            if (player->tv_width > 0) {
                argv[argn++] = g_strdup ("-tv:width");
                argv[argn++] = g_strdup_printf("%i", player->tv_width);
            }
            if (player->tv_height > 0) {
                argv[argn++] = g_strdup ("-tv:height");
                argv[argn++] = g_strdup_printf("%i", player->tv_height);
            }
            if (player->tv_fps > 0) {
                argv[argn++] = g_strdup ("-tv:fps");
        ...
        player->std_out = -1;
        player->std_err = -1;
        spawn = g_spawn_async_with_pipes(NULL, argv, NULL,
...
static void finalize_mplayer(GmtkMediaPlayer * player)
{
    ...
    g_source_remove(player->watch_in_hup_id);
    g_source_remove(player->watch_err_id);
    remove (player->af_export_filename);
...
gboolean thread_reader_error(GIOChannel * source, GIOCondition condition, gpointer data)
{
    ...
    if (status == G_IO_STATUS_ERROR) {
        ...
        if (strstr(mplayer_output->str, "Failed to open") != NULL) {
            if (strstr(mplayer_output->str, "LIRC") == NULL &&
                ...
                if (strstr(mplayer_output->str, "<") == NULL && strstr(mplayer_output->str, ">") == NULL
                    && player->type == TYPE_FILE) {
                    error_msg =
                        g_strdup_printf(gettext("Failed to open %s"),
        ...
        if (strstr(mplayer_output->str, "Compressed SWF format not supported") != NULL) {
            error_msg = g_strdup_printf(gettext("Compressed SWF format not supported"));
        }
        ...
        if (error_msg != NULL && player->playback_error == NO_ERROR) {
            ...
            /*
               ...
               gtk_window_set_title(GTK_WINDOW(dialog), gettext("LXplayer Error"));
...
gboolean thread_reader(GIOChannel * source, GIOCondition condition, gpointer data)
{
    ...
    if (status == G_IO_STATUS_ERROR) {
        ...
        if (strstr(mplayer_output->str, "Cache fill") != 0) {
            ...
            buf = g_strdup_printf(gettext("Cache fill: %2.2f%%"), percent);
        ...
        if (strstr(mplayer_output->str, "ID_SUBTITLE_ID=") != 0) {
            ...
            subtitle->id = id;
            subtitle->lang = g_strdup (gettext("Unknown"));
            subtitle->name = g_strdup (gettext("Unknown"));
            ...
            if (subtitle) {
                ...
                subtitle->label =
                    g_strdup_printf("%s (%s) - %i",
                                    (subtitle->name) ? subtitle->name : gettext("Unknown"),
        ...
        if (strstr(mplayer_output->str, "ID_FILE_SUB_ID=") != 0) {
            ...
            subtitle->label = g_strdup_printf(gettext("External Subtitle #%i"), id + 1);
            player->subtitles = g_list_append(player->subtitles, subtitle);
            create_event_int(player, "subtitles-changed", g_list_length(player->subtitles));
        }
        ...
        if (strstr(mplayer_output->str, "ID_AUDIO_ID=") != 0) {
            ...
            if (!found) {
                ...
                audio_track->id = id;
                audio_track->lang = g_strdup (gettext("Unknown"));
                audio_track->name = g_strdup (gettext("Unknown"));
        ...
        if (strstr(mplayer_output->str, "ID_AID_") != 0) {
            ...
            if (audio_track) {
                ...
                audio_track->label =
                    g_strdup_printf("%s (%s) - %i",
                                    (audio_track->name) ? audio_track->name : gettext("Unknown"),
        ...
        if (strstr(mplayer_output->str, "*** screenshot") != 0) {
            ...
            buf[12] = '\0';
            message = g_strdup_printf(gettext("Screenshot saved to '%s'"), buf);
            dialog = gtk_message_dialog_new(NULL, GTK_DIALOG_DESTROY_WITH_PARENT, GTK_MESSAGE_INFO,
                                            GTK_BUTTONS_OK, "%s", message);
            gtk_window_set_title(GTK_WINDOW(dialog), gettext("LXplayer Notification"));
            gtk_dialog_run(GTK_DIALOG(dialog));
            gtk_widget_destroy(dialog);
            g_free(message);
            message = NULL;
        ...
        if (strstr(mplayer_output->str, "failed (forgot -vf screenshot?)") != 0) {
            dialog = gtk_message_dialog_new(NULL, GTK_DIALOG_DESTROY_WITH_PARENT, GTK_MESSAGE_ERROR,
                                            GTK_BUTTONS_OK, gettext("Failed to take screenshot"));
            gtk_window_set_title(GTK_WINDOW(dialog), gettext("LXplayer Notification"));
            gtk_dialog_run(GTK_DIALOG(dialog));
            gtk_widget_destroy(dialog);
        }
