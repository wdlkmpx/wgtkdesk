...
gboolean detect_playlist(gchar * uri)
{
    ...
    gint size;
#endif
    if (strncasecmp(uri, "cdda://", strlen("cdda://")) == 0) {
        if (strlen(uri) > strlen("cdda://")) {
            playlist = FALSE;
        } else {
            playlist = TRUE;
        }
    } else if (strncasecmp(uri, "dvd://", strlen("dvd://")) == 0) {
        if (strlen(uri) > strlen("dvd://")) {
            playlist = FALSE;
        } else {
            playlist = TRUE;
        }
    } else if (strncasecmp(uri, "vcd://", strlen("vcd://")) == 0) {
...
gboolean parse_playlist(gchar * uri)
{
    ...
    ret = parse_basic(uri);
    if (!ret) ret = parse_ram(uri);
    if (!ret) ret = parse_cdda(uri);
    if (!ret) ret = parse_dvd(uri);
    if (!ret) ret = parse_vcd(uri);
...
gboolean parse_basic(gchar * uri)
{
    ...
    if (fp != NULL) {
        while (!feof(fp)) {
            ...
            if ((strncasecmp(line, "ref", 3) == 0) || (strncasecmp(line, "file", 4)) == 0) {
            ...
            if (strncasecmp(newline, "[playlist]", strlen("[playlist]")) == 0) {
                gm_log(verbose, G_LOG_LEVEL_DEBUG, "playlist");
                //continue;
            } else if (strncasecmp(newline, "[reference]", strlen("[reference]")) == 0) {
                gm_log(verbose, G_LOG_LEVEL_DEBUG, "ref");
                //continue;
                //playlist = TRUE;
            } else if (strncasecmp(newline, "<smil", strlen("<smil")) == 0) {
                ...
                g_free(newline);
                break;
            } else if (strncasecmp(newline, "numberofentries", strlen("numberofentries")) == 0) {
                gm_log(verbose, G_LOG_LEVEL_DEBUG, "num");
                //continue;
            } else if (strncasecmp(newline, "version", strlen("version")) == 0) {
                gm_log(verbose, G_LOG_LEVEL_DEBUG, "ver");
                //continue;
            } else if (strncasecmp(newline, "title", strlen("title")) == 0) {
                gm_log(verbose, G_LOG_LEVEL_DEBUG, "title");
                //continue;
            } else if (strncasecmp(newline, "length", strlen("length")) == 0) {
                gm_log(verbose, G_LOG_LEVEL_DEBUG, "len");
                //continue;
            } else if (strncasecmp(newline, "#extinf", strlen("#extinf")) == 0) {
                gm_log(verbose, G_LOG_LEVEL_DEBUG, "#extinf");
                //continue;
            } else if (strncasecmp(newline, "#", strlen("#")) == 0) {
        ...
        while (!feof(fp)) {
            if (fgets(buffer, 1024, fp) != NULL) {
                ...
                while (output[ac] != NULL) {
                    ...
                    g_strchug(output[ac]);
                    if (strncasecmp(output[ac], "rtsp://", strlen("rtsp://")) == 0) {
                        ret = TRUE;
                        add_item_to_playlist(output[ac], FALSE);
                    } else if (strncasecmp(output[ac], "pnm://", strlen("pnm://")) == 0) {
                        ret = TRUE;
                        add_item_to_playlist(output[ac], FALSE);
                    }
                    ac++;
...
gboolean parse_cdda(gchar * filename)
{
    ...
    if (strncasecmp(filename, "cdda://", 7) != 0) {
        return FALSE;
    } else {
        playlist = FALSE;
        ...
        while (output[ac] != NULL) {
            ...
            if (strncasecmp(output[ac], " artist", strlen(" artist")) == 0) {
            ...
            if (strncasecmp(output[ac], " album", strlen(" album")) == 0) {
        ...
        if (addcount == 0) {
            ac = 0;
            while (output[ac] != NULL) {
                if (strncasecmp(output[ac], "ID_CDDA_TRACK__", strlen("ID_CDDA_TRACK_")) == 0) {
...
gboolean parse_dvd(gchar * filename)
{
    ...
    if (strncasecmp(filename, "dvd://", strlen("dvd://")) == 0) {       // || strncasecmp(filename,"dvdnav://",strlen("dvdnav://")) == 0) {
        playlist = FALSE;
        ...
        ac = 0;
        while (output[ac] != NULL) {
            if (strncasecmp(output[ac], "ID_DVD_TITLE_", strlen("ID_DVD_TITLE_")) == 0) {
        ...
        if (error_msg != NULL) {
            ...
            gtk_window_set_title(GTK_WINDOW(dialog), _("LXplayer Error"));
...
gboolean parse_vcd(gchar * filename)
{
    ...
    if (strncasecmp(filename, "vcd://", strlen("vcd://")) == 0) {
        playlist = FALSE;
        ...
        ac = 0;
        while (output[ac] != NULL) {
            if (strncasecmp(output[ac], "ID_VCD_TRACK_", strlen("ID_VCD_TRACK_")) == 0) {
...
gboolean streaming_media(gchar * uri)
{
    ...
    if (device_name(uri)) {
        ret = FALSE;
    } else if (strncasecmp(uri, "http://", strlen("http://")) == 0) {
        ret = TRUE;
    } else if (strncasecmp(uri, "mmst://", strlen("mmst://")) == 0) {
        ret = TRUE;
    } else if (strncasecmp(uri, "mms://", strlen("mms://")) == 0) {
        ret = TRUE;
    } else if (strncasecmp(uri, "mmshttp://", strlen("mmshttp://")) == 0) {
        ret = TRUE;
    } else {
#ifdef GIO_ENABLED
        ...
        if (local_file != NULL) {
            ...
            if (file != NULL) {
                ...
                if (info != NULL) {
                    ...
                    if (strncasecmp(uri, "smb://", strlen("smb://")) == 0) {
...
{
    gboolean ret = FALSE;
    if ((strncasecmp(filename, "dvd://", strlen("dvd://")) == 0)
        || (strncasecmp(filename, "dvdnav://", strlen("dvdnav://")) == 0)
        || (strncasecmp(filename, "cdda://", strlen("cdda://")) == 0)
        || (strncasecmp(filename, "cddb://", strlen("cddb://")) == 0)
        || (strncasecmp(filename, "tv://", strlen("tv://")) == 0)
        || (strncasecmp(filename, "dvb://", strlen("dvb://")) == 0)
        || (strncasecmp(filename, "vcd://", strlen("vcd://")) == 0))
    {
        ret = TRUE;
    }
    if (ret) {
...
MetaData *get_basic_metadata(gchar * uri)
{
    ...
    if (title == NULL && strncasecmp(name, "dvd://", strlen("dvd://")) == 0) {
        localtitle = g_strrstr(name, "/") + 1;
        title = g_strdup_printf("DVD Track %s", localtitle);
    }

    if (title == NULL && strncasecmp(name, "tv://", strlen("tv://")) == 0) {
        localtitle = g_strrstr(name, "/") + 1;
        title = g_strdup_printf("%s", localtitle);
    }

    if (title == NULL && strncasecmp(name, "dvb://", strlen("dvb://")) == 0) {
        localtitle = g_strrstr(name, "/") + 1;
        title = g_strdup_printf("%s", localtitle);
    }

    if (title == NULL && strncasecmp(name, "vcd://", strlen("vcd://")) == 0) {
        localtitle = g_strrstr(name, "/") + 1;
        title = g_strdup_printf("%s", localtitle);
    }

    if (title == NULL && strncasecmp(name, "dvdnav://", strlen("dvdnav://")) == 0) {
...
MetaData *get_metadata(gchar * uri)
{
    ...
    if (strncasecmp(name, "dvb://", strlen("dvb://")) == 0
        || strncasecmp(name, "tv://", strlen("tv://")) == 0) {
    ...
    while (output != NULL && output[ac] != NULL) {
        ...
        if (strstr(output[ac], "_LENGTH") != NULL
            && (strncasecmp(name, "dvdnav://", strlen("dvdnav://")) != 0
                || strncasecmp(name, "dvd://", strlen("dvd://")) != 0)) {
            localtitle = strstr(output[ac], "=") + 1;
            sscanf(localtitle, "%f", &seconds);
            length = seconds_to_string(seconds);
        }

        if (strncasecmp(output[ac], "ID_CLIP_INFO_NAME", strlen("ID_CLIP_INFO_NAME")) == 0) {
...
gboolean add_item_to_playlist(const gchar * uri, gboolean playlist)
{
    ...
    if (data == NULL || !data->valid) {
        ...
        } else if (strncasecmp(uri, "cdda://", strlen("cdda://")) == 0) {
            data = (MetaData *) g_new0(MetaData, 1);
            data->title = g_strdup_printf("CD Track %s", uri + strlen("cdda://"));
            data->playable = TRUE;
        } else if (device_name(local_uri)) {
            if (strncasecmp(uri, "dvdnav://", strlen("dvdnav://")) == 0) {
                loop = 1;
            }
            retrieve = TRUE;
            if (strncasecmp(uri, "dvb://", strlen("dvb://")) == 0) {
                retrieve = FALSE;
            }
            if (strncasecmp(uri, "tv://", strlen("tv://")) == 0) {
                retrieve = FALSE;
            }
            data = get_basic_metadata(local_uri);
...
gboolean save_playlist_pls(gchar * uri)
{
    ...
    if (data != NULL) {
        ...
        gtk_window_set_title(GTK_WINDOW(dialog), "LXplayer Error");
        gtk_dialog_run(GTK_DIALOG(dialog));
        gtk_widget_destroy(dialog);
...
gchar *get_localfile_from_uri(gchar * uri)
{
    ...
    if (localfile == NULL) {
        ...
        tmp = g_strdup_printf("%s/lxplayer", g_get_user_cache_dir());
...
#else
                g_cond_wait (idledata->caching_complete, g_static_mutex_get_mutex (&(idledata->caching)));
#endif
