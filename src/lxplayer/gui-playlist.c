...
void add_folder_to_playlist(GtkWidget * widget, void *data)
{
    ...
    gchar *last_dir;
    gchar *message;
    int count = gtk_tree_model_iter_n_children(GTK_TREE_MODEL(playliststore), NULL);
...
void create_playlist_widget()
{
    ...
    playlist_set_subtitle = w_gtk_menuitem_new (&menuitem);

    menuitem.label       = _("Set Audi_o");
    menuitem.activate_cb = playlist_set_audiofile_callback;
    menuitem.cb_data     = list;
    playlist_set_audiofile = w_gtk_menuitem_new (&menuitem);
    //-
