...
void main_win_init( MainWin*mw )
{
    ...
    {
        gtk_window_set_icon_from_file((GtkWindow*)mw, PACKAGE_DATADIR "/icons/hicolor/48x48/apps/gpicview.png", NULL);
    }
...
void create_nav_bar( MainWin* mw, GtkWidget* box )
{
    ...
    add_nav_btn( mw, "gtk-go-back", _("Previous"), G_CALLBACK(on_prev), FALSE );
    add_nav_btn( mw, "gtk-go-forward", _("Next"), G_CALLBACK(on_next), FALSE );
    mw->btn_play_stop = add_nav_btn_img( mw, "gtk-media-play", _("Start Slideshow"), G_CALLBACK(on_slideshow), TRUE, &mw->img_play_stop );

    gtk_box_pack_start( (GtkBox*)mw->nav_bar, gtk_separator_new (GTK_ORIENTATION_VERTICAL), FALSE, FALSE, 0 );

    add_nav_btn( mw, "gtk-zoom-out", _("Zoom Out"), G_CALLBACK(on_zoom_out), FALSE );
    add_nav_btn( mw, "gtk-zoom-in", _("Zoom In"), G_CALLBACK(on_zoom_in), FALSE );
...
//    gtk_box_pack_start( (GtkBox*)nav_bar, percent, FALSE, FALSE, 2 );
    ...
    mw->btn_fit = add_nav_btn( mw, "gtk-zoom-fit", _("Fit Image To Window Size"),
                           G_CALLBACK(on_zoom_fit), TRUE );
    mw->btn_orig = add_nav_btn( mw, "gtk-zoom-100", _("Original Size"),
                           G_CALLBACK(on_orig_size), TRUE );
    gtk_toggle_button_set_active( (GtkToggleButton*)mw->btn_fit, TRUE );

    add_nav_btn( mw, "gtk-fullscreen", _("Full Screen"), G_CALLBACK(on_full_screen), FALSE );   // gtk+ 2.8+
    ...
    add_nav_btn( mw, "gtk-open", _("Open File"), G_CALLBACK(on_open), FALSE );
    add_nav_btn( mw, "gtk-save", _("Save File"), G_CALLBACK(on_save), FALSE );
    add_nav_btn( mw, "gtk-save-as", _("Save File As"), G_CALLBACK(on_save_as), FALSE );
    add_nav_btn( mw, "gtk-delete", _("Delete File"), G_CALLBACK(on_delete), FALSE );

    gtk_box_pack_start( (GtkBox*)mw->nav_bar, gtk_separator_new (GTK_ORIENTATION_VERTICAL), FALSE, FALSE, 0 );
    add_nav_btn( mw, "gtk-preferences", _("Preferences"), G_CALLBACK(on_preference), FALSE );
    add_nav_btn( mw, "gtk-quit", _("Quit"), G_CALLBACK(on_quit), FALSE );
...
void on_slideshow( GtkToggleButton* btn, MainWin* mw )
{
    if ((mw->slideshow_running) || (mw->slideshow_cancelled))
    {
        ...
        gtk_image_set_from_stock( GTK_IMAGE(mw->img_play_stop), "gtk-media-play", GTK_ICON_SIZE_SMALL_TOOLBAR );
    ...
    else
    {
        ...
        gtk_image_set_from_stock( GTK_IMAGE(mw->img_play_stop), "gtk-media-stop", GTK_ICON_SIZE_SMALL_TOOLBAR );
...
gboolean on_key_press_event(GtkWidget* widget, GdkEventKey * key)
{
    ...
    switch( key->keyval )
    {
        ...
        case GDK_KEY(Down):
        case GDK_KEY(downarrow):
            on_next( NULL, mw );
            break;
        case GDK_KEY(Left):
        case GDK_KEY(KP_Left):
        case GDK_KEY(leftarrow):
            if( gtk_widget_get_default_direction () == GTK_TEXT_DIR_RTL )
                on_next( NULL, mw );
            else
                on_prev( NULL, mw );
...
void show_popup_menu( MainWin* mw, GdkEventButton* evt )
{
    ...
    {
        PTK_IMG_MENU_ITEM( N_( "Previous" ), "gtk-go-back", on_prev, GDK_KEY(leftarrow), 0 ),
        PTK_IMG_MENU_ITEM( N_( "Next" ), "gtk-go-forward", on_next, GDK_KEY(rightarrow), 0 ),
        PTK_IMG_MENU_ITEM( N_( "Start/Stop Slideshow" ), "gtk-media-play", on_slideshow_menu, GDK_KEY(W), 0 ),
        PTK_SEPARATOR_MENU_ITEM,
        PTK_IMG_MENU_ITEM( N_( "Zoom Out" ), "gtk-zoom-out", on_zoom_out, GDK_KEY(minus), 0 ),
        PTK_IMG_MENU_ITEM( N_( "Zoom In" ), "gtk-zoom-in", on_zoom_in, GDK_KEY(plus), 0 ),
        PTK_IMG_MENU_ITEM( N_( "Fit Image To Window Size" ), "gtk-zoom-fit", on_zoom_fit_menu, GDK_KEY(F), 0 ),
        PTK_IMG_MENU_ITEM( N_( "Original Size" ), "gtk-zoom-100", on_orig_size_menu, GDK_KEY(G), 0 ),
        PTK_SEPARATOR_MENU_ITEM,
        PTK_IMG_MENU_ITEM( N_( "Full Screen" ), "gtk-fullscreen", on_full_screen, GDK_KEY(F11), 0 ),
        PTK_SEPARATOR_MENU_ITEM,
        PTK_IMG_MENU_ITEM( N_( "Rotate Counterclockwise" ), "object-rotate-left", on_rotate_counterclockwise, GDK_KEY(L), 0 ),
        PTK_IMG_MENU_ITEM( N_( "Rotate Clockwise" ), "object-rotate-right", on_rotate_clockwise, GDK_KEY(R), 0 ),
        PTK_IMG_MENU_ITEM( N_( "Flip Horizontal" ), "object-flip-horizontal", on_flip_horizontal, GDK_KEY(H), 0 ),
        PTK_IMG_MENU_ITEM( N_( "Flip Vertical" ), "object-flip-vertical", on_flip_vertical, GDK_KEY(V), 0 ),
        PTK_SEPARATOR_MENU_ITEM,
        PTK_IMG_MENU_ITEM( N_("Open File"), "gtk-open", G_CALLBACK(on_open), GDK_KEY(O), 0 ),
        PTK_IMG_MENU_ITEM( N_("Save File"), "gtk-save", G_CALLBACK(on_save), GDK_KEY(S), 0 ),
        PTK_IMG_MENU_ITEM( N_("Save As"), "gtk-save-as", G_CALLBACK(on_save_as), GDK_KEY(A), 0 ),
//        PTK_IMG_MENU_ITEM( N_("Save As Other Size"), "gtk-save-as", G_CALLBACK(on_save_as), GDK_KEY(A), 0 ),
        PTK_IMG_MENU_ITEM( N_("Delete File"), "gtk-delete", G_CALLBACK(on_delete), GDK_KEY(Delete), 0 ),
        PTK_SEPARATOR_MENU_ITEM,
        PTK_IMG_MENU_ITEM( N_("Preferences"), "gtk-preferences", G_CALLBACK(on_preference), GDK_KEY(P), 0 ),
        PTK_IMG_MENU_ITEM( N_("Show/Hide toolbar"), NULL, G_CALLBACK(on_toggle_toolbar), GDK_KEY(T), 0 ),
        PTK_STOCK_MENU_ITEM( "gtk-about", on_about ),
        PTK_SEPARATOR_MENU_ITEM,
        PTK_IMG_MENU_ITEM( N_("Quit"), "gtk-quit", G_CALLBACK(on_quit), GDK_KEY(Q), 0 ),
        PTK_MENU_END
    };
    GtkWidget* rotate_cw;
...
void on_about( GtkWidget* menu, MainWin* mw )
{
    ...
    gtk_about_dialog_set_website ( (GtkAboutDialog*)about_dlg, PACKAGE_URL );
